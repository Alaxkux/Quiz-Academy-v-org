...
import { Star, BookOpen, Shuffle, Dumbbell, Search, Bot, Trophy, Calendar } from 'lucide-react'
import { getTodayProgress, isDailyChallengeCompleted } from '../../utils/gamification'

function ActionTile({ emoji, icon: Icon, label, sub, onClick, accent, badge, progress, delay = 0 }) {
  return (
    <motion.button
      onClick={onClick}
      className="flex flex-col items-center justify-center gap-2 p-3 rounded-2xl text-center w-full transition-all"
      style={{
        background:  'var(--bg1)',
    ...
    >
      ...
      {sub && <div className="text-xs text-muted leading-tight" style={{ fontSize: 10 }}>{sub}</div>}

      {/* Study planner progress ring */}
      {progress !== undefined && (
        <div className="relative w-8 h-8 flex-shrink-0 mt-1">
          ...
          <span className="absolute inset-0 flex items-center justify-center font-display font-bold"
...
export default function QuickActions({ user, hasInProgress = false }) {
  ...
  const actions = [
    ...
    },
    {
      ...
      onClick: () => navigate('/play?tab=review'), delay: 0.25,
    },
    {
      emoji: '🤖', label: 'AI Questions',   sub: 'Generate with Gemini',
      onClick: () => navigate('/ai'),        delay: 0.3, badge: 'New',
    },
    {
      emoji: '🏆', label: 'Leaderboard',    sub: 'Global rankings',
      onClick: () => navigate('/leaderboard'), delay: 0.35,
    },
    {
      emoji: '📅',
      label: 'Study Planner',
